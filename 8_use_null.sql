...
-- 2. Note the INNER JOIN misses the teachers with no department and departments with no teacher
...
-- 6. Use the COALESCE function and a LEFT JOIN to print the teacher name and dept name, use 'none' when no dept
SELECT teacher.name, COALESCE(dept.name, 'None') AS department
FROM teacher LEFT JOIN dept ON (teacher.dept = dept.id);

-- 7. Use COUNT to show the number of teachers and the number of mobile phones
SELECT COUNT(id) AS teachers, COUNT(mobile) AS phones
FROM teacher;
...
SELECT teacher.name, 
CASE
WHEN teacher.dept = 1 OR teacher.dept = 2 THEN 'Sci' ELSE 'Art' END AS dept
FROM teacher;
